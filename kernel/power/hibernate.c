...
int hibernate(void)
{
	...
	pm_standby_transition(PM_STANDBY_SLEEP, false);
#endif
...
#if CONFIG_SUSPEND
	/*
	 * If we powered down, current standby state will be active but
	 * the suspend module will remember the previous state so we need
	 * to sync it. After hibernation, standby state will always be active.
	 * 
	 * If the hibernation fails or we performed a snapshot test, we need
	 * to restore the user's standby state.
	 */
	if (powered_down)
